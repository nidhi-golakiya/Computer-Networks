...
int main(int argc, char * argv[])
{
  ...
  if ((setsockopt(s, IPPROTO_IP, IP_ADD_MEMBERSHIP, 
    ...
    memset(buf, 0, sizeof(buf));
		...
		for(x = 1;x<=6;x++)	// Continuously transmitting english songs
		{
			char str1[20];
			char str2[100];
			char sn[20];
			...
			if(x == 1)
			{
				strcpy(sname,"Thousand years");
				strcpy(name,"I don't wanna live forever");
			}
			else if(x == 2)
			{
				strcpy(sname,"I don't wanna live forever");
				strcpy(name,"Love me like you do");
			}
			else if(x == 3)
			{
				strcpy(sname,"Love me like you do");
				strcpy(name,"Perfect");
			}
			else if(x == 4)
			{
				strcpy(sname,"Perfect");
				strcpy(name,"Faded");
			}
			else if(x == 5)
			{
				strcpy(sname,"Faded");
				strcpy(name,"I have a dream");
			}
			else
			{
				strcpy(sname,"I have a dream");
				strcpy(name,"Thousand years");
			}
