...
int main(int argc, char * argv[])
{
  ...
  if ((setsockopt(s, IPPROTO_IP, IP_ADD_MEMBERSHIP, 
    ...
    memset(buf, 0, sizeof(buf));
		...
		for(x=7;x<=10;x++)	// Continuously transmitting english songs
		{
			char str1[20];
			char str2[100];
			char sn[20];
			...
			if(x == 7)
			{
				strcpy(sname,"Aahatein");
				strcpy(name,"Ajeeb Dasta Hai Yeh");
			}
			else if(x == 8)
			{
				strcpy(sname,"Ajeeb Dasta Hai Yeh");
				strcpy(name,"Ek Pyaar Ka Nagma");
			}
			else if(x == 9)
			{
				strcpy(sname,"Ek Pyaar Ka Nagma");
				strcpy(name,"Dariya");
			}
			else
			{
				strcpy(sname,"Dariya");
				strcpy(name,"Aahatein");
			}
